...
float ripple(vec2 uv, float dirSign, float ampMul, float rotAngle, float phaseShift) {
  ...
  for (int i = 0; i < ${MAX_POINTS}; i++) {
    ...
    float freq = 10.0;
    float speed = 3.0;
    float decay = 2.6; // gentler spatial falloff — a broad, soft swell
...
float rippleField(vec2 uv) {
  float h1 = ripple(uv, 1.0, 1.0, 0.0, 0.0);
  float h2 = ripple(uv, -1.0, 0.22, 0.35, 3.14159);
...
void main() {
  ...
  float spec = pow(max(dot(normal, lightDir), 0.0), 6.0);
  color.rgb += spec * 0.42;
  color.rgb += max(h, 0.0) * 0.06;
  color.rgb *= 1.0 - clamp(-h * 0.3, 0.0, 0.18);
